...
function Watchlist() {
  const { user, getWatchlist, removeFromWatchlist: removeFromWatchlistAPI } = useContext(AuthContext);
  ...
  }, [user, navigate, API_KEY, getWatchlist]);
  ...
  const removeFromWatchlist = async (movieId, movieTitle) => {

    setIsRemoving((prev) => ({ ...prev, [movieId]: true }));
    try {
      const result = await removeFromWatchlistAPI(String(movieId));
  ...
  if (movies.length === 0) {
    return (
      <main className="min-h-screen bg-black text-white overflow-hidden">
        <div
          ref={emptyStateRef}
          className="container mx-auto px-4 py-16 mt-8 bg-black text-white-custom"
        >
          ...
          >
            <div>
              ...
              <p className="text-gray-400 mt-1">
                {movies.length} movies saved to watch later
              </p>
            </div>
          </div>
              ...
              <div key={movie.id} className="group relative">
                <div className="relative aspect-[2/3] rounded-lg overflow-hidden">
                  ...
                  <div className="absolute top-2 right-2 z-10 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button
                      className="h-8 w-8 rounded-full bg-black/50 hover:bg-black/70 text-white-custom flex items-center justify-center"
                      onClick={(e) => {
                        e.preventDefault();
